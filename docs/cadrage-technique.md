# Cadrage technique — carte de France WebGL/WebGPU temps réel

> Note de cadrage initiale du dépôt, écrite avant le projet **Archives** (voir le [README](../readme.md)). Elle reste la référence pour l'architecture des données temps réel ; certaines pistes de rendu (tuiles vectorielles MapLibre) ont depuis cédé la place au relief IGN en direct, décrit dans `docs/superpowers/specs/`.
